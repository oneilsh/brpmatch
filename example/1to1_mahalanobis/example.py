"""
Example: 1:1 Matching with Mahalanobis Distance

Demonstrates 1:1 propensity score matching using Mahalanobis distance.
...
def main():
    """
    Demonstrates standard 1:1 propensity score matching using BRPMatch.

    Workflow:
        1. Load raw patient data
        2. Generate features (transform and standardize covariates)
        3. Match treated to control patients using LSH-based k-NN
    ...
    Each treated patient is matched to exactly one control patient.
    """
    print(section_header("1:1 Matching - Mahalanobis Distance", "="))

    # Create Spark session
    spark = create_spark_session("brpmatch-1to1-mahalanobis")
    ...
    units, pairs, bucket_stats = match(features_df, feature_space="mahalanobis", n_neighbors=10, verbose=False)
