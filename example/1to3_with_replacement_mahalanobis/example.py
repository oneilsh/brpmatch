"""
Example: 1:3 Matching WITH Replacement - Mahalanobis Distance

Demonstrates 1:3 matching where each treated gets their 3 nearest controls.
Controls can be reused. Mahalanobis distance accounts for correlations.
"""
...
def main():
    """
    Demonstrates 1:3 propensity score matching WITH replacement using BRPMatch.
    ...
    Each treated patient is matched to exactly 3 control patients (their k-nearest neighbors).
    With replacement: controls can be reused multiple times across different treated patients.
    """
    print(section_header("1:3 Matching WITH Replacement - Mahalanobis Distance", "="))

    # Create Spark session
    spark = create_spark_session("brpmatch-1to3-with-repl-mahalanobis")
    ...
    #
    # With ratio_k=3 and with_replacement=True:
    #   - Each treated patient gets exactly 3 control matches (their k-nearest neighbors)
    #   - Each control CAN be matched to multiple treated patients (with replacement)
    #   - Popular controls may be matched many times, less similar ones fewer times
    #   - pair_weight automatically accounts for control reuse in analysis
    #
    ...
    # Show distribution of matches per treated (should be constant at ratio_k with replacement)
    ...
    # Show control reuse statistics (key difference from without replacement)
    control_usage = matched.groupBy("match_id").count().toPandas()
    print(f"  Control reuse: {(control_usage['count'] > 1).sum()} controls used multiple times")
    ...
    #   - control_usage_count: how many times this control was matched (may be > 1 with replacement)
